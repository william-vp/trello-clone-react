...
exports.getBoards = async (req, res) => {
    ...
    let boards = await Board.find({
        $or: [
            {visibility: 'public'},
            {user: user._id, visibility: 'private'}
        ]
    });

    let boards_user= Array();
    boards_user= boards.filter(async (board) => {
        let members = await MemberBoard.find({board: board._id, user: user._id});
        if (members.length > 0) {
            return board;
        }
    });
    return res.status(200).json({boards: boards_user, code: "success"});
}

exports.searchUserBoards = async (req, res) => {
    const {uid} = req.user;
    ...
    let boards = await Board.find({
        $or: [
            {"name": {$regex: `.*${query}.*`, $options: 'i'}, visibility: 'public'},
            {"name": {$regex: `.*${query}.*`, $options: 'i'}, visibility: 'private', user: user._id },
            {"description": {$regex: `.*${query}.*`, $options: 'i'}, visibility: 'public'},
            {"description": {$regex: `.*${query}.*`, $options: 'i'}, visibility: 'private', user: user._id},
        ]
    });
