import {
    ...
    SET_ORDER_CARDS_LIST, SET_BOARD_SEARCH
...
export function setBoardsSearchAction(order) {
    return async (dispatch) => {
        try {
            dispatch(setBoardsSearch(order))
        } catch (e) {
        }
    }
}
...
const setBoardsSearch = query => ({
    type: SET_BOARD_SEARCH, payload: query
});