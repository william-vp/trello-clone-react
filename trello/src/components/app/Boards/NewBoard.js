import React, {useState} from 'react';
import {Button, Form, Image, Input, Modal, Tooltip} from "antd";
import {useTranslation} from "react-i18next";
import {CloseOutlined, PlusOutlined} from '@ant-design/icons'
import CoverBoardSelect from "./CoverBoardSelect";
import {Lock as LockIcon, LockOpen as LockOpenIcon} from "@material-ui/icons";
import {useDispatch, useSelector} from "react-redux";
import {createBoardAction} from "../../../actions/boardActions";

const modalStyle = {
    boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.05)!important',
    borderRadius: '8px!important'
}

const NewBoard = () => {
    const {t} = useTranslation(['app', 'board'])

    const dispatch = useDispatch();
    const {loadingBoard} = useSelector(state => state.board);

    const createBoard = board => dispatch(createBoardAction(board))

    const [visible, setVisible] = useState(false);

    const [form, setForm] = useState({
        name: '',
        visibility: false,
        cover: null,
        coverUrl: null
    });
    const {name, visibility, cover} = form

    const handleShowModal = () => setVisible(true)

    const handleCancel = e => {
        console.log(e);
        setVisible(false)
    }

    const onChange = (e) => {
        const field = e.target.name
        const value = e.target.value
        setForm({
            ...form, [field]: value
        })
    }

    const onChangeImage = (cover, srcFull) => {
        setForm({
            ...form, cover, coverUrl: srcFull
        })
    }

    const submitNewBoard = async () => {
        createBoard(form)
        setForm({
            name: '',
            visibility: false,
            cover: null,
            coverUrl: null
        })
        setVisible(false)
    }

    return (
        <div>
            <Button
                onClick={handleShowModal}
                icon={<PlusOutlined/>}
                type="primary"
                size={"medium"}>{t('new_board')}</Button>
            <Modal
                cancelButtonProps={{type: 'text', size: 'large', className: "mb-4", disabled: loadingBoard}}
                cancelText={t('cancel_text')}
                okText={t('create_text')}
                okButtonProps={{icon: <PlusOutlined/>, size: 'large', loading: loadingBoard, onClick: submitNewBoard}}
                closeIcon={<div/>}
                //wrapClassName="wrapper-modal"
                bodyStyle={modalStyle}
                visible={visible}
                onCancel={handleCancel}>
                <Button onClick={handleCancel} type="primary" className="button-close">
                    <CloseOutlined/>
                </Button>
                <div className="text-center mt-3">
                    {cover ?
                        <Image
                            className="img-rounded-board"
                            width={'100%'}
                            height={250}
                            alt="cover image"
                            src={cover}/>
                        : <Image
                            className="img-rounded-board"
                            width={'100%'}
                            height={250}
                            src={"error"}
                            fallback="data:image/png;base64,"
                        />
                    }
                </div>

                <Form
                    className="mt-3"
                    name="new_board"
                    initialValues={{name}}
                    onFinish={submitNewBoard}>
                    <Form.Item
                        required
                        rules={[{
                            required: true,
                            message: `${t('message_input')} ${t('label_name')}`
                        }]}>
                        <Input
                            disabled={loadingBoard}
                            value={name}
                            onChange={onChange}
                            name="name"
                            className="input-name-board"
                            placeholder={` ${t('board:placeholder_name')}`}/>
                    </Form.Item>

                    <div className="row mt-5 mb-4">
                        <div className="col-6">
                            <CoverBoardSelect onChangeImage={onChangeImage}/>
                        </div>
                        <div className="col-6">
                            <Tooltip title={t('visibility_message')}>
                                <Button
                                    disabled={loadingBoard}
                                    block
                                    onClick={() => setForm({...form, visibility: !visibility})}
                                    type="primary" size="large"
                                    className={!visibility ? "bg-gray border-0 rounded-lg p-1 text-gray px-3" : "border-0 rounded-lg p-1 px-3"}>
                                    {visibility ? <>
                                        <LockOpenIcon className="m-icon"/> {t('public_label')}
                                    </> : <>
                                        <LockIcon className="m-icon"/> {t('private_label')}
                                    </>}
                                </Button>
                            </Tooltip>
                        </div>
                    </div>
                </Form>
            </Modal>
        </div>
    );
};
export default NewBoard;